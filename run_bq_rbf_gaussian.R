...
hyp <- readRDS("results/hyperparams_rbf_gaussian.rds")
ell <- hyp$posterior$ell_mean
sigma_f2 <- hyp$posterior$sigmaf2_mean
n <- 150
mean_plot_start <- 70L
n_max_mean_plot <- 150L
tot_n <- 100L
mc_seed <- 1000L
s_draws_ci <- 100L
...
prior_var <- rbf_prior_var_gaussian(ell = ell, sigma_f2 = sigma_f2)
...
run_std <- function() {
  run_bq(
    ...
    kernel_mean_fun = rbf_kernel_mean_gaussian,
    prior_var = prior_var,
    ell = ell,
    sigma_f2 = sigma_f2
  )
...
saveRDS(
  ...
  file = "results/bq_rbf_gaussian.rds"
)
...
plot_bq_posterior_mean_bands(
  ...
  outfile = "figures/bq_rbf_gaussian_mean_bands.pdf"
)
...
plot_bq_posterior_variance_log(
  ...
  label_optimal = label_opt,
  label_standard = label_std,
  outfile = "figures/bq_rbf_gaussian_variance.pdf"
)
...
plot_bq_averaged_mean_ci(
  ...
  iter_start = mean_plot_start,
  n_max = n_max_mean_plot,
  outfile = "figures/bq_rbf_gaussian_mean_ci_avg.pdf"
)
...
plot_bq_averaged_variance_log(
  ...
  label_optimal = label_opt,
  label_standard = label_std,
  outfile = "figures/bq_rbf_gaussian_variance_avg.pdf"
)
